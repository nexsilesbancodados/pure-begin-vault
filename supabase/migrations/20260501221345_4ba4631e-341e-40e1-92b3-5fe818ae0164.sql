...
BEGIN
    ...
    -- Limpar nome: descartar IDs internos do WhatsApp e nomes genéricos bugados
    v_clean_name := CASE 
        WHEN _name IS NOT NULL 
             AND _name <> '' 
             AND _name <> _phone 
             AND _name <> v_normalized_phone 
             -- Não pode ser um ID longo (ex: cmonbjx...)
             AND _name !~ '^[a-zA-Z0-9]{15,}$'
             -- Não pode ser apenas números
             AND _name !~ '^\d+$'
             -- Não pode conter carácteres típicos de IDs do WhatsApp
    ...
    ELSE
        ...
        -- Se já existe, atualizar nome se tivermos um nome LIMPO e VÁLIDO e o atual for genérico ou bugado
        IF v_clean_name IS NOT NULL AND (
            ...
            v_current_name = '' OR
            (v_current_name <> v_clean_name AND LENGTH(v_clean_name) > 2 AND v_current_name !~ '^[A-Z][a-z]+ [A-Z][a-z]+') -- Atualiza se não parecer um nome real completo
        ) THEN
            UPDATE public.leads SET name = v_clean_name WHERE id = v_lead_id;
        END IF;
    END IF;

    -- Pegar a etapa inicial do funil do usuário
    ...
    IF v_stage_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            INSERT INTO public.pipeline_leads (user_id, lead_id, stage_id, instance_name, deal_value, priority)
            VALUES (_user_id, v_lead_id, v_stage_id, _instance_name, 0, 'medium');
        ELSE
