    ...
    const fetchStats = useCallback(async () => {
      ...
      try {
        ...
        switch (period) {
          ...
          case 'last30': startDate = startOfDay(subDays(now, 30)); break;
        ...
        const salesQ = orgId
          ? supabase.from("sales_orders").select("total_amount, created_at, status").eq("organization_id", orgId)
        ...
        const [salesRes, productsRes, leadsRes, osRes] = await Promise.all([salesQ, productsQ, leadsQ, osQ]);
        ...
        const isCompleted = (s: string | null | undefined) => s === 'completed' || s === 'concluded' || s === 'paid';
        ...
        const monthSales = sales.filter((s: any) => new Date(s.created_at!) >= firstDayMonth && isCompleted(s.status));
        const monthRevenue = monthSales.reduce((acc: number, curr: any) => acc + (Number(curr.total_amount) || 0), 0);

        const lowStockCount = products.filter((p: any) => (Number(p.stock_quantity) || 0) <= (Number(p.min_stock) || 5)).length;
        ...
        const activeOSCount = os.filter((o: any) => o.status !== 'delivered' && o.status !== 'canceled' && o.status !== 'cancelled').length;

        const avgTicket = monthSales.length > 0 ? monthRevenue / monthSales.length : 0;

        setStats({ todaySales, monthRevenue, activeOS: activeOSCount, lowStock: lowStockCount, newLeads: newLeadsCount, avgTicket });
      } catch (error) {
