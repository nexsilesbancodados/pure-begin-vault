...
export function DealCard({ deal, onRemove, onDragStart, onDragEnd, onClick }: DealCardProps) {
  ...
  const isStale =
    ...
    new Date().getTime() - new Date(deal.last_message_at).getTime() > 24 * 60 * 60 * 1000;
              ...
              {deal.tags && deal.tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {deal.tags.map((tag: string, i: number) => (
                    <span
                      key={i}
                      className="text-[9px] font-black uppercase bg-primary/10 text-primary px-1.5 py-0.5 rounded-md flex items-center gap-1"
                    >
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span
              className={cn(
                ...
                isNewFromUser ? "text-green-600" : "text-muted-foreground/70",
              )}
            >
          ...
          {deal.last_message_at && (
            <span className="text-[10px] text-muted-foreground/60 font-bold flex items-center gap-1 bg-background/50 px-1.5 py-0.5 rounded-md">
              ...
              {formatDistanceToNow(new Date(deal.last_message_at), {
                addSuffix: true,
                locale: ptBR,
              })}
            </span>
        ...
        <p
          className={cn(
            ...
            isNewFromUser
              ? "text-foreground font-extrabold line-clamp-3"
              : "text-foreground/70 line-clamp-2",
          )}
        >
      ...
      <div className="flex items-center justify-between mt-1 pt-3 border-t border-border/40 relative z-10">
        ...
        <div className="flex flex-col items-end gap-1.5">
          <div className="flex items-center gap-1.5">
            {deal.notes && (
              <div
                className="h-5 w-5 rounded-md bg-amber-100 flex items-center justify-center text-amber-600"
                title="Possui notas"
              >
