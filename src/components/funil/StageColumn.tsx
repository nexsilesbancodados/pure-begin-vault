...
export function StageColumn({
  ...
  return (
    <div
      ...
      onDrop={() => {
        if (dragId) onMoveDeal(dragId, stage.id);
        setDragId(null);
      }}
      className={cn(
      ...
      <div className="p-4 flex flex-col gap-2 relative group">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5 min-w-0">
            <div
              className="h-3 w-3 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.1)] ring-2 ring-background"
              style={{ backgroundColor: stage.color ?? "#888" }}
            />
            <h3 className="text-[11px] font-black uppercase tracking-[0.15em] text-foreground/70 truncate">
              {stage.name || "Sem Nome"}
            </h3>
          </div>
