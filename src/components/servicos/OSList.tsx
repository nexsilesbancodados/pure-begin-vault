  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a
          href="/servicos/nova"
          className="h-10 px-5 rounded-xl bg-gradient-primary text-white flex items-center gap-2 text-sm font-bold shadow-glow hover:opacity-95 transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          >
            <div
              className={`h-12 w-12 rounded-xl bg-muted/50 grid place-items-center ${stat.color}`}
            >
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((os) => (
                <tr key={os.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tight border ${statusColors[os.status || "open"] || "bg-muted"}`}
                    >
