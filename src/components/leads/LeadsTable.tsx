...
export function LeadsTable() {
  ...
  const save = async () => {
    ...
    if (!orgId) {
      toast.error("Crie ou selecione uma loja antes de cadastrar leads.");
      return;
    }
    ...
    if (!editing.id && inserted?.id) {
      fireAutomation(
        user.id,
        "new_lead",
        { lead_id: inserted.id, phone: inserted.phone, email: inserted.email },
        orgId,
      );
      notify({
        user_id: user.id,
  ...
  const handleQuickSave = async () => {
    ...
    if (inserted?.id) {
      fireAutomation(user.id, "new_lead", { lead_id: inserted.id, phone: inserted.phone }, orgId);
      notify({
        user_id: user.id,
        organization_id: orgId,
        type: "lead_new",
  ...
  const statusBadge = (s: string | null) => {
    ...
    return (
      <span
        className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border ${cfg.className}`}
      >
        {cfg.label}
      </span>
    );
          ...
          <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
            <div className="p-5 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className={`gap-2 ${isQuickAddOpen ? "bg-muted text-foreground hover:bg-muted/80" : ""}`}
                >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-muted/30">
                    <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground border-b border-border">
                      Nome / Contato
                    </th>
                    <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground border-b border-border">
                      Canal
                    ...
                    <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground border-b border-border text-right">
                      Ações
                    </th>
                  </tr>
                </thead>
                      ...
                      <tr key={lead.id} className="hover:bg-muted/20 transition group">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold group-hover:text-primary transition">
                                {lead.name}
                              </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            {lead.source === "instagram" ? (
                              <div className="h-7 w-7 rounded-lg bg-pink-500/10 grid place-items-center">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1">
                            {lead.whatsapp_tags?.map((tag, i) => (
                              <span
                                key={i}
                                className="px-1.5 py-0.5 rounded-md bg-primary/10 text-primary text-[9px] font-black uppercase"
                              >
                                {tag}
                              </span>
                            ))}
                            {(!lead.whatsapp_tags || lead.whatsapp_tags.length === 0) && (
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {lead.phone && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="h-8 w-8 grid place-items-center rounded-lg hover:bg-success/10 hover:text-success transition text-muted-foreground"
                              >
                            ...
                            <button
                              ...
                              className="h-8 w-8 grid place-items-center rounded-lg hover:bg-primary/10 hover:text-primary transition text-muted-foreground"
                            >
                              <Pencil className="h-4 w-4" />
                            </button>
                            <button
                              onClick={() => remove(lead.id)}
                              className="h-8 w-8 grid place-items-center rounded-lg hover:bg-destructive/10 hover:text-destructive transition text-muted-foreground"
                            >
            ...
            <div className="p-5 border-t border-border flex items-center justify-between bg-muted/10">
              <div className="text-xs text-muted-foreground">
                ...
                <span className="font-bold text-foreground">{leads.length}</span> leads
              </div>
            </div>
          </div>
        </main>
