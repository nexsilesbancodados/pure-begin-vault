...
export function Funnel() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: deals } = await filt(
        supabase.from("pipeline_leads").select(`*, leads (name, source)`),
      );
      ...
      const formattedStages = (stgs || []).map((s: any) => {
        ...
        return {
          ...
          total: stageDeals
            .reduce((acc: number, curr: any) => acc + (Number(curr.deal_value) || 0), 0)
            ...
            stages.map((s) => (
              <div key={s.id} className="rounded-xl bg-muted/40 border border-border p-3">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {s.label.toLowerCase().includes("ganho") && (
                ...
                <ul className="space-y-2">
                  {s.leads.map((l: any, idx: number) => (
                    <li
                      key={`${s.id}-${idx}`}
                      className="flex items-center gap-2.5 rounded-lg bg-card border border-border p-2 hover:shadow-card transition"
                    >
                ...
                {s.count > 3 && (
                  <button
                    ...
                    className="mt-3 w-full text-[11px] text-muted-foreground hover:text-primary transition"
                  >
                    + Ver mais {s.count - 3}
                  </button>
                )}
