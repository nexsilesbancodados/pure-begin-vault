      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {recentMessages.length > 0 ? (
            ...
            <li className="text-center py-8 text-xs text-muted-foreground italic border border-dashed border-border rounded-xl">
...
const etapaStyle = (e: string) => {
  switch (e) {
    ...
    case "Em Atendimento":
      return "bg-warning/20 text-[oklch(0.5_0.15_75)]";
    case "Proposta":
              ...
              {leads.length > 0 ? (
                  ...
                  <tr key={l.id || idx} className="hover:bg-muted/40 transition">
                    ...
                    <td className="py-2.5">
                      <span className="inline-flex items-center gap-1.5 text-foreground/80">
                        <span
                          className={`h-2 w-2 rounded-full ${l.source === "WhatsApp" ? "bg-success" : "bg-primary"}`}
                        />
                        {l.source || "Direto"}
                      </span>
                    </td>
                    <td className="py-2.5">
                      <span
                        className={`text-[10.5px] font-semibold px-2 py-0.5 rounded ${etapaStyle(l.etapa)}`}
                      >
                        {l.etapa}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-xs text-muted-foreground italic">
                    Sem leads captados recentemente
                  </td>
                </tr>
              )}
            </tbody>
