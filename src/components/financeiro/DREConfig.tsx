...
export function DREConfig() {
  ...
  const dre = useMemo(() => {
    const grupo = (cats: string[], isIncome: boolean) => {
      return txs
        .filter((t) => {
          const matchType = isIncome ? (t.type === "income" || t.type === "receita") : (t.type === "expense" || t.type === "despesa");
    ...
    const administrativas = grupo(["administrativ", "aluguel", "salario", "luz", "internet", "telefone"], false);
    ...
    return { receitaBruta, impostos, receitaLiquida, cpv, lucroBruto, operacionais, administrativas, ebitda, depreciacao, lucroLiquido };
  }, [txs]);
  ...
  const exportPdf = () => {
    Export.pdf(`DRE — ${MONTH_NAMES[month]}/${year}`, rows.map((r) => ({ ...r, value_formatted: BRL(r.value) })), [
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm font-medium">Calculado em tempo real a partir do Fluxo de Caixa</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={exportPdf} className="h-9 gap-2 font-bold rounded-xl">
      ...
      {loading ? (
        <Card className="p-12 flex justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /></Card>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card className="p-5 rounded-2xl">
              ...
              <div className="mt-2 text-[10px] font-bold text-slate-400">{txs.filter(t => t.type === "income" || t.type === "receita").length} lançamentos</div>
            </Card>
            <Card className="p-5 rounded-2xl">
              ...
              <div className="mt-2 text-[10px] font-bold text-slate-400">Lucro Bruto: {BRL(dre.lucroBruto)}</div>
            </Card>
            <Card className="p-5 rounded-2xl">
              ...
              <div className="mt-2 text-[10px] font-bold text-slate-400">{dre.receitaBruta > 0 ? ((despesasFixas / dre.receitaBruta) * 100).toFixed(1) : "0,0"}% da receita</div>
            </Card>
            <Card className="p-5 border-none shadow-lg bg-slate-900 text-white rounded-2xl">
              ...
              <div className={`text-2xl font-black ${dre.lucroLiquido < 0 ? "text-red-400" : ""}`}>{BRL(dre.lucroLiquido)}</div>
              <div className="mt-2 text-[10px] font-bold text-blue-400">Margem Líquida: {margemLiquida.toFixed(1)}%</div>
            </Card>
          </div>
          ...
          <Card className="rounded-2xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
              <div>
                ...
                <CardDescription>{MONTH_NAMES[month]}/{year} · {txs.length} lançamentos</CardDescription>
              </div>
              <Link to="/financeiro/plano-contas">
                <Button variant="outline" size="sm" className="h-8 gap-1 text-[10px] font-black uppercase rounded-lg">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/50 dark:bg-slate-900/50">
                    <TableHead className="px-6 py-3 text-[10px] font-black uppercase tracking-widest">Descrição</TableHead>
                    <TableHead className="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-right">Valor</TableHead>
                    <TableHead className="px-6 py-3 text-[10px] font-black uppercase tracking-widest text-right">% Receita</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {rows.map((item, idx) => {
                    const pct = dre.receitaBruta > 0 ? (Math.abs(item.value) / dre.receitaBruta) * 100 : 0;
                    return (
                      <TableRow key={idx} className={`${item.type === "total" ? "bg-slate-50/50 dark:bg-slate-900/50 font-bold" : ""} ${item.type === "final" ? "bg-slate-100/80 dark:bg-slate-800/80 font-black" : ""}`}>
                        <TableCell className="px-6 py-3 text-sm">{item.label}</TableCell>
                        <TableCell className={`px-6 py-3 text-right text-sm font-bold ${item.value < 0 ? "text-red-600" : item.type === "revenue" || item.type === "final" || item.type === "total" ? "text-green-600" : ""}`}>
                          {BRL(item.value)}
                        </TableCell>
