...
export function TransactionForm({ open, onOpenChange, onSave, transaction }: TransactionFormProps) {
    ...
    if (transaction) {
      setFormData({
        ...
        payment_methods: transaction.payment_methods || [
          { method: transaction.payment_method || "Pix", amount: transaction.amount || "" },
        ],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.payment_methods.length > 1) {
      const totalSplit = formData.payment_methods.reduce(
        ...
        >
          <div className="px-6 border-b">
            <TabsList className="w-full justify-start h-12 bg-transparent p-0 gap-6">
              <TabsTrigger
                value="geral"
                className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-blue-600 rounded-none px-0 h-12 text-xs font-bold uppercase tracking-wider"
              >
                    ...
                    {isAddingCategory ? (
                      ...
                      >
                        ...
                        <SelectContent className="rounded-xl border-slate-200 shadow-xl overflow-hidden">
                          <ScrollArea className="h-[200px]">
                            {categories.map((cat) => (
                              <SelectItem
                                key={cat}
                                value={cat}
                                className="focus:bg-blue-50 focus:text-blue-700 rounded-lg mx-1"
                              >
                                {cat}
                              </SelectItem>
                            ))}
                          ...
                          <SelectItem
                            value="add_new"
                            className="text-blue-600 font-black focus:bg-blue-600 focus:text-white rounded-lg mx-1 cursor-pointer"
                          >
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="space-y-3">
                    {formData.payment_methods.map((pm: any, index: number) => (
                      <div
                        key={index}
                        className="flex items-end gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 animate-in fade-in slide-in-from-right-2"
                      >
              ...
              <TabsContent value="anexos" className="mt-0 space-y-6">
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground font-medium">
                      Comprovantes, Notas Fiscais, PDFs, etc.
                    </p>
                  </div>
                  <input
                    type="file"
                ...
                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 flex gap-3">
                  ...
                  <p className="text-[11px] text-amber-700 font-medium">
                    Arquivos anexados serão salvos e vinculados a este lançamento para consulta
                    futura.
                  </p>
                </div>
              </TabsContent>
