...
export function Quotations() {
  ...
  const load = async () => {
    ...
    const { data } = await (supabase as any)
      ...
      .limit(100);
    setItems(
      (data ?? []).map((q: any) => ({ ...q, items: Array.isArray(q.items) ? q.items : [] })),
    );
    setLoading(false);
  };
  useEffect(() => {
  ...
  const filtered = items.filter((i) =>
    !q
      ? true
      : (i.customer_name + (i.customer_phone ?? "")).toLowerCase().includes(q.toLowerCase()),
  );
              ...
              {filtered.map((qu) => (
                <tr key={qu.id} className="border-b hover:bg-muted/20">
                  <td className="p-3">
                    ...
                    {qu.customer_phone && (
                      <p className="text-xs text-muted-foreground">{qu.customer_phone}</p>
                    )}
                  </td>
                  ...
                  <td className="p-3">
                    <Badge className={statusColor(qu.status)}>{qu.status}</Badge>
                  </td>
                  <td className="p-3 text-xs text-muted-foreground">
                    {qu.expires_at ? new Date(qu.expires_at).toLocaleDateString("pt-BR") : "—"}
                  </td>
                  <td className="p-3 text-right whitespace-nowrap">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => convertToSale(qu)}
                      title="Converter em venda"
                    >
                      <ShoppingCart className="h-3.5 w-3.5" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => remove(qu.id)} title="Excluir">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Garantia, condições, etc"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button
              onClick={save}
              disabled={saving || !form.customer_name?.trim() || draftItems.length === 0}
            >
              {saving ? "Salvando..." : `Criar (${BRL(subtotal)})`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
