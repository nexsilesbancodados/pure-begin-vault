...
export const getOrgSummaries = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.orgIds.length === 0) return { organizations: {} as Record<string, { name: string | null; logo_url: string | null }> };
    ...
    const allowedIds = new Set(((memberships as { organization_id: string }[]) ?? []).map((m) => m.organization_id));
    ...
    if (ids.length === 0) return { organizations: {} as Record<string, { name: string | null; logo_url: string | null }> };

    const [{ data: orgRows }, { data: settingsRows }] = await Promise.all([
      supabaseAdmin.from("organizations").select("id, name").in("id", ids),
      supabaseAdmin.from("organization_settings").select("organization_id, brand_logo_url").in("organization_id", ids),
    ]);

    const logos = new Map(((settingsRows as { organization_id: string; brand_logo_url: string | null }[]) ?? []).map((row) => [row.organization_id, row.brand_logo_url]));
