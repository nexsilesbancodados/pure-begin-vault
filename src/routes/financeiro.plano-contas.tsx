...
function FinancePlanoContasPage() {
      ...
      const roots = [
        {
          ...
          code: "1",
          type: "revenue",
          description: "Entradas da atividade principal",
        },
        {
          user_id: user.id,
          name: "CUSTOS OPERACIONAIS (CPV/CSP)",
          code: "2",
          type: "expense",
          description: "Custos diretos de vendas/serviços",
        },
        {
          user_id: user.id,
          ...
          code: "3",
          type: "expense",
          description: "Gastos fixos de operação",
        },
        {
          user_id: user.id,
          ...
          code: "4",
          type: "expense",
          description: "Marketing, comissões e fretes",
        },
      ];
  ...
  function AccountItem({ account, allAccounts }: { account: any; allAccounts: any[] }) {
    ...
    const content = (
      <div
        className={`group flex items-center justify-between ${isRoot ? "px-5 py-4" : "px-4 py-2 hover:bg-slate-50 rounded-lg transition-all"}`}
      >
        <div className="flex items-center gap-3">
          {hasChildren ? (
            ...
            !isRoot && <div className="w-6 h-px bg-slate-200" />
          )}

          {isRoot ? (
            <div
              className={`h-8 w-8 rounded-lg flex items-center justify-center ${account.type === "revenue" ? "bg-green-100 text-green-600" : account.type === "expense" ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-600"}`}
            >
            ...
            <div
              className={`h-2 w-2 rounded-full ${account.type === "revenue" ? "bg-green-400" : "bg-red-400"}`}
            />
          )}

          <div className="flex flex-col">
            <span
              className={`tracking-tight ${isRoot ? "font-black text-sm text-slate-900" : "font-bold text-xs text-slate-600 group-hover:text-slate-900"}`}
            >
              {account.code} - {account.name}
            </span>
            {account.description && (
              <span className="text-[10px] text-slate-400 font-medium">{account.description}</span>
            )}
          </div>
        </div>

        <div
          className={`flex items-center gap-1 ${!isRoot && "opacity-0 group-hover:opacity-100 transition-opacity"}`}
        >
          <Button
            variant="ghost"
            size="icon"
    ...
    if (isRoot) {
      return (
        <Card
          className={`border-border shadow-sm overflow-hidden rounded-2xl mb-4 ${account.type === "revenue" ? "bg-green-50/10" : account.type === "expense" ? "bg-red-50/10" : ""}`}
        >
          <div
            className={`border-b border-slate-100 ${account.type === "revenue" ? "bg-green-50/30" : account.type === "expense" ? "bg-red-50/30" : "bg-slate-50/30"}`}
          >
            {content}
          </div>
          {isExpanded && (
    ...
    <div className="min-h-screen flex w-full bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar
          ...
          subtitle="Estrutura de categorização financeira"
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              ...
              <div className="flex gap-2">
                ...
                <SeedDreButton
                  onDone={() => queryClient.invalidateQueries({ queryKey: ["chart_of_accounts"] })}
                />
                <Button
