...
function SuppliersPage() {
  ...
  const filtered = items.filter((s) =>
    !q
      ? true
      : (s.name + (s.cnpj ?? "") + (s.contact_name ?? "") + (s.city ?? ""))
          .toLowerCase()
            ...
            <Card>
              <EmptyState
                ...
                description={
                  q
                    ? "Tente outra busca."
                    : "Cadastre fornecedores pra registrar entradas de NF e relatório de compras."
                }
                  ...
                  {filtered.map((s) => (
                    <tr key={s.id} className="border-b border-border hover:bg-muted/20">
                      ...
                      <td className="p-3 text-right">
                        <button
                          onClick={() => openEdit(s)}
                          className="h-8 w-8 grid place-items-center rounded-lg hover:bg-muted inline-flex"
                        >
                          <Edit2 className="h-3.5 w-3.5" />
                        </button>
                        <button
                          onClick={() => remove(s.id)}
                          className="h-8 w-8 grid place-items-center rounded-lg hover:bg-destructive/10 hover:text-destructive inline-flex ml-1"
                        >
