...
function GrowthReports() {
  ...
  const [marketingSpend, setMarketingSpend] = useState(() =>
    Number(localStorage.getItem("marketing_spend_30") ?? "1000"),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const ltv =
          paying > 0
            ? won.reduce((s: number, l: any) => s + (Number(l.deal_value) || 0), 0) / paying
            : 0;
        ...
        const byStage = (stages ?? []).map((s: any) => {
          ...
          return {
            name: s.name,
            count,
            pct: totalLeads > 0 ? count / totalLeads : 0,
            color: s.color,
          };
        });
        ...
        byLead.forEach((ms) => {
          ...
          const outbound = ms.find(
            (m: any) =>
              m.direction === "outbound" &&
              new Date(m.created_at) > new Date(inbound?.created_at ?? 0),
          );
          if (inbound && outbound)
            respTimes.push(
              (+new Date(outbound.created_at) - +new Date(inbound.created_at)) / 60000,
            );
        });
  ...
  return (
    <div className="min-h-screen flex w-full bg-background">
      ...
      <div className="flex-1 flex flex-col">
        <Topbar
          title="Crescimento — CAC, LTV & conversão"
          subtitle="Métricas que importam pra decidir investir mais ou menos"
        />
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-card border border-border rounded-2xl p-4 flex items-center gap-4 flex-wrap">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setMarketingSpend(v);
                localStorage.setItem("marketing_spend_30", String(v));
              }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Kpi
              icon={DollarSign}
              label="Receita 30d"
              value={fmtCurrency(stats.revenue30)}
              tone="success"
            />
            <Kpi
              icon={Users}
              label="CAC"
              value={fmtCurrency(stats.cac)}
              sub={`${stats.totalLeads} leads totais`}
              tone="info"
            />
            <Kpi
              icon={TrendingUp}
              label="LTV (ticket médio cliente)"
              value={fmtCurrency(stats.ltv)}
              sub={`${stats.paying} clientes pagantes`}
              tone="primary"
            />
            <Kpi
              icon={Target}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Kpi
              ...
              sub={`${stats.paying} de ${stats.totalLeads}`}
              tone="success"
            />
            <Kpi
              icon={Clock}
              ...
              value={
                stats.avgResponseMin > 60
                  ? `${(stats.avgResponseMin / 60).toFixed(1)}h`
                  : `${stats.avgResponseMin.toFixed(0)} min`
              }
              sub={
                stats.avgResponseMin <= 5
                  ? "✅ Excelente"
                  : stats.avgResponseMin <= 30
                    ? "👍 Bom"
                    : "⚠️ Lento — perde lead"
              }
              tone="warning"
            ...
            <Kpi
              icon={Repeat}
              label="Churn mensal"
              value={fmtPct(stats.monthlyChurn)}
              sub="Em breve com pagamentos"
              tone="destructive"
            />
          </div>
            ...
            <div className="space-y-3">
                ...
                <div key={s.name}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-muted-foreground">
                      {s.count} leads · {fmtPct(s.pct)}
                    </span>
                  </div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${s.pct * 100}%`, background: s.color || "#3b82f6" }}
                    />
                  </div>
                </div>
              ))}
              {stats.byStage.length === 0 && (
                <div className="text-sm text-muted-foreground text-center py-8">
                  Configure as etapas do funil para ver a conversão
                </div>
              )}
            </div>
          </div>
