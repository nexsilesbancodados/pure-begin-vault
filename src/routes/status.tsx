...
function StatusPage() {
  ...
  const run = async () => {
    ...
    for (const c of CHECKS) {
      ...
      try {
        ...
        out.push({ ...c, status: "down", error: e.message, latency_ms: Math.round(performance.now() - t0) });
      }
    }
    setChecks(out);
    setLastRun(new Date());
  ...
  const overall = checks.every((c) => c.status === "ok") ? "ok" : checks.some((c) => c.status === "down") ? "down" : "warn";
    ...
    <div className="min-h-screen bg-gradient-to-br from-background to-muted/30 p-4 md:p-8">
      <div className="max-w-2xl mx-auto space-y-4">
        <div className="text-center">
          <p className="text-xs uppercase tracking-widest text-muted-foreground font-black">ConectaCRM</p>
        ...
        <div className="space-y-2">
          {checks.map((c) => {
            ...
            const Icon = c.status === "ok" ? CheckCircle2 : c.status === "warn" ? AlertCircle : c.status === "down" ? XCircle : Activity;
            return (
              <div key={c.name} className={`rounded-2xl border p-4 ${cls}`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <Icon className={`h-5 w-5 shrink-0 ${c.status === "ok" ? "text-success" : c.status === "warn" ? "text-warning" : c.status === "down" ? "text-destructive" : "text-muted-foreground"}`} />
                  ...
                  <div className="text-right text-xs">
                    ...
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground">
                      {c.status === "ok" ? "Operacional" : c.status === "warn" ? "Atenção" : c.status === "down" ? "Indisponível" : "—"}
                    </p>
                  </div>
                </div>
              </div>
