...
export const Route = createFileRoute("/assinar")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Crie sua conta após o pagamento. Checkout seguro via Mercado Pago.",
      },
        ...
        {step === "info" && plan && (
          <Card className="max-w-md mx-auto">
            <CardHeader>
              ...
              <CardDescription>
                Plano <strong>{plan.name}</strong> — {brl(plan.price_cents)}/
...
function PaymentBrick({
  ...
  return (
    <div>
      ...
      <Button variant="outline" size="sm" className="mt-4" onClick={onBack} disabled={submitting}>
        Voltar
      </Button>
    </div>
  );
    ...
    return (
      <div className="text-center">
        ...
        <Button asChild className="mt-6 w-full">
          <a href={result.account.action_link}>Acessar minha conta</a>
        </Button>
        <p className="text-xs text-muted-foreground mt-3">
          Também enviamos um link de acesso para o seu e-mail.
        </p>
      </div>
    );
  }
      ...
      <div className="text-center">
        ...
        {result.qr_code_base64 && (
          <img
            ...
            className="w-64 h-64 mx-auto mt-4 border rounded"
          />
        )}
        ...
        <div className="mt-6 p-3 rounded bg-muted text-left text-sm flex gap-2">
          ...
          <span>
            Assim que confirmarmos o pagamento, enviaremos um link de acesso para{" "}
            <strong>{email}</strong>.
          </span>
        </div>
      </div>
  ...
  if (result.boleto_url) {
    return (
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Após o pagamento, sua conta será liberada e enviaremos o acesso por e-mail.
        </p>
  ...
  if (result.status === "pending" || result.status === "in_process") {
    return (
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Avisaremos por e-mail ({email}) assim que for aprovado.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="text-center">
      <h3 className="text-lg font-semibold text-destructive">
        Pagamento {result.status || "recusado"}
      </h3>
      <p className="text-sm text-muted-foreground mt-1">
      ...
      <Button variant="ghost" onClick={navigateLogin} className="mt-2">
        Já tenho conta
      </Button>
    </div>
  );
