...
export const Route = createFileRoute("/equipe")({
  head: () => ({
    meta: [
      { title: "Equipe — ConectaCRM" },
      { name: "description", content: "Gerencie permissões e membros" },
    ],
  }),
...
function EquipePage() {
  ...
  const handleInvite = async (newMember: any) => {
    ...
    try {
      const role =
        newMember.role.toLowerCase() === "administrador"
          ? "admin"
          : newMember.role.toLowerCase() === "financeiro"
            ? "financeiro"
            : newMember.role.toLowerCase() === "vendedor"
              ? "vendedor"
              : "employee";
    ...
    <div className="min-h-screen flex w-full bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar
          ...
          subtitle="Gerencie as permissões dos seus membros"
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {team.map((member) => (
                  <div
                    key={member.id}
                    className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all group relative overflow-hidden"
                  >
                    {member.id === user?.id && (
                      <div className="absolute top-0 right-0">
                        <div className="bg-indigo-500 text-white text-[9px] font-black px-3 py-1 rounded-bl-xl tracking-widest uppercase">
                          VOCÊ
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-start justify-between mb-6">
                      <div className="relative">
                        ...
                        <div
                          className={`absolute -bottom-1 -right-1 h-5 w-5 rounded-full border-4 border-white ${member.id === user?.id ? "bg-emerald-500" : "bg-slate-300"}`}
                        />
                      </div>
                    </div>
                    ...
                    <div className="mb-6">
                      <h3 className="font-bold text-lg text-slate-900 group-hover:text-indigo-600 transition-colors">
                        {member.display_name || "Membro sem nome"}
                      </h3>
