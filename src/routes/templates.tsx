    ...
    <div className="min-h-screen flex w-full bg-background">
      ...
      <div className="flex-1 flex flex-col">
        <Topbar
          ...
          subtitle="Templates com mídia para WhatsApp e Instagram"
        />
        <main className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm text-muted-foreground">
              {list.length} modelo(s) salvos. Use{" "}
              <code className="px-1 bg-muted rounded">{"{{nome}}"}</code> para variáveis.
            </div>
            <button
              onClick={openNew}
              className="h-10 px-4 rounded-xl bg-primary text-primary-foreground font-bold text-sm inline-flex items-center gap-2 hover:opacity-90"
            >
          ...
          {loading ? (
            ...
            <div className="bg-card border border-border rounded-2xl p-10 text-center">
              ...
              <div className="text-sm text-muted-foreground mb-4">
                Crie modelos de mensagem com texto, imagem, vídeo ou documento
              </div>
              <button
                onClick={openNew}
                className="h-10 px-4 rounded-xl bg-primary text-primary-foreground font-bold text-sm"
              >
              ...
              {list.map((t) => (
                ...
                >
                    ...
                    <div className="mb-2 rounded-lg overflow-hidden bg-muted aspect-video grid place-items-center">
                      {t.media_type === "image" ? (
                        ...
                        <div className="text-xs text-muted-foreground flex items-center gap-1">
                          {mediaIcon(t.media_type)} {t.media_type || "mídia"}
                        </div>
                      )}
                    </div>
                  )}
                  <div className="text-sm text-muted-foreground whitespace-pre-wrap line-clamp-4 flex-1">
                    {t.body}
                  </div>
                  {Array.isArray(t.variables) && t.variables.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {t.variables.map((v: string) => (
                        <span
                          key={v}
                          className="text-[10px] inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-muted text-muted-foreground"
                        >
                  ...
                  <div className="flex gap-1 mt-3 pt-3 border-t border-border">
                    <button
                      onClick={() => copyBody(t)}
                      className="flex-1 h-8 rounded-lg border border-border text-xs font-bold hover:bg-muted inline-flex items-center justify-center gap-1"
                    >
                    ...
                    <button
                      onClick={() => openEdit(t)}
                      className="flex-1 h-8 rounded-lg border border-border text-xs font-bold hover:bg-muted"
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => remove(t.id)}
                      className="h-8 w-8 grid place-items-center rounded-lg border border-border hover:bg-destructive/10 hover:text-destructive"
                    >
      ...
      {open && (
        ...
        >
          <div
            className="bg-card border border-border rounded-2xl p-6 w-full max-w-lg space-y-3 my-8"
          ...
          >
            ...
            {form.body && detectVars(form.body).length > 0 && (
              <div className="text-xs text-muted-foreground">
                ...
                {detectVars(form.body).map((v) => (
                  <span key={v} className="ml-1 px-1.5 py-0.5 rounded bg-muted font-mono">
                    {v}
                  </span>
                ))}
              </div>
