...
function AgentsPage() {
  ...
  const load = async () => {
    ...
    const { data } = await (supabase as any)
      ...
      .select(
        "user_id, role, is_default, user:profiles!user_organizations_user_id_fkey(id, display_name, email, role, avatar_url)",
      )
              ...
              <div className="flex items-center gap-2">
                <Link to="/crm/bot">
                  <Button variant="secondary" className="gap-2">
                    <Zap className="h-4 w-4" /> Configurar bot
                  </Button>
                </Link>
              </div>
            </div>
            ...
            <Card>
              <EmptyState
                ...
                action={{
                  ...
                  onClick: () => (window.location.href = "/equipe-loja"),
                }}
              />
            </Card>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-bold truncate">
                        {a.user?.display_name ?? a.user?.email ?? "Sem nome"}
                      </p>
                      {a.role === "owner" && (
