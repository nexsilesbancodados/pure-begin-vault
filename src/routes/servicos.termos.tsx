...
function WarrantyPage() {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ category: CATEGORIES[0], days: 90, content: defaultContent(90), active: true });
    setOpen(true);
  };
  ...
  const save = async () => {
    if (!form.name?.trim() || !form.content?.trim() || !orgId || !userId)
      return toast.error("Preencha nome e conteúdo");
    setSaving(true);
    const payload = {
      ...form,
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 items-end">
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => setForm({ ...form, content: defaultContent(form.days ?? 90) })}
              >
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Texto que será impresso..."
              />
              <p className="text-[11px] text-muted-foreground mt-1">
                Use as variáveis: {"{cliente}, {produto}, {data}, {dias}, {loja}"}
              </p>
            </div>
          </div>
          <DialogFooter>
