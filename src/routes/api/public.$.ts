...
const ENDPOINTS: Record<
  string,
  { method: string; handler: (orgId: string, params: any) => Promise<any> }
> = {
  "v1/customers": {
    method: "GET",
    handler: async (orgId) => {
  ...
  "v1/products": {
    ...
    handler: async (orgId) => {
      const { data } = await (supabaseAdmin as any)
        .from("products")
        .select("id, name, sku, ean, price, stock_quantity, category")
        .eq("organization_id", orgId)
        .eq("active", true)
        .limit(1000);
      return { data: data ?? [] };
  ...
  "v1/sales": {
    ...
    handler: async (orgId) => {
      const { data } = await (supabaseAdmin as any)
        .from("sales_orders")
        .select("id, sale_number, total_amount, status, payment_method, created_at, customer_id")
        .eq("organization_id", orgId)
  ...
  "v1/orders": {
    ...
    handler: async (orgId) => {
      const { data } = await (supabaseAdmin as any)
        .from("service_orders")
        .select("id, os_number, equipment, status, customer_id, created_at, total_cost")
        .eq("organization_id", orgId)
        .order("created_at", { ascending: false })
        .limit(500);
      return { data: data ?? [] };
  ...
  "v1/stats": {
    method: "GET",
    handler: async (orgId) => {
      const [{ count: customers }, { count: products }, { count: sales }, { count: orders }] =
        await Promise.all([
          (supabaseAdmin as any)
            .from("customers")
